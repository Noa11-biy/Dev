...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        new fr.javacraft.input.InputManager(window);
        ...
        // 6) Camera (no player arg — camera manages its own position)
        ...
        while (running && !GLFW.glfwWindowShouldClose(window)) {
            ...
            vp.mul(proj, view);
            float[] vpArr = vp.get(new float[16]);
            gameRenderer.updateFrustum(vpArr);
            ...
            // === UPLOAD MATRICES TO SHADER (fix: terrain was invisible without these) ===
            gameRenderer.uploadMatrices(proj, view);
